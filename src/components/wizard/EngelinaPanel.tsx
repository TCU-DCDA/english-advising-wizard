...
// --- Prompt chip pools (ported from Sandra app.js) ---
...
function getSessionId(): string {
  let id = localStorage.getItem('engelina-session-id')
  if (!id) {
    id = crypto.randomUUID()
    localStorage.setItem('engelina-session-id', id)
  }
  return id
}
...
export function EngelinaPanel({ open, onClose, wizardContext, programName, programId, department = 'English' }: EngelinaPanelProps) {
  const personaName = PERSONA_NAMES[department] || 'Engelina'
  ...
  const storageKey = `engelina-history-${programId || department}`
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    try {
      const response = await fetch(ENGELINA_API_URL, {
        method: 'POST',
  ...
  const handleFeedback = async (messageIndex: number, rating: 'positive' | 'negative') => {
    ...
    try {
      await fetch(ENGELINA_FEEDBACK_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
